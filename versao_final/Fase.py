...
class Fase:
    ...
    def iniciar(self):
        ...
        #inicializa as imagens dos obstaculos
        for obstaculo in self.__obstaculos:
            obstaculo.imagem = pygame.image.load(obstaculo.imagem).convert_alpha()
            obstaculo.imagem = pygame.transform.scale(obstaculo.imagem, (70, 70))

        #inicializa as imagens do jogador
        self.__jogador.imagem = pygame.image.load(self.__jogador.imagem).convert_alpha()
        self.__jogador.imagem = pygame.transform.scale(self.__jogador.imagem, (80, 40))
        ...
        horizontal, vertical = self.__jogador.imagem.get_size()
        ...
        #pega o rect da imagem  de todos obstaculos
        obstaculos_rect = []
        for obstaculo in self.__obstaculos:
            obstaculos_rect.append(obstaculo.imagem.get_rect())
        ...
        while rodando:
            ...
            if contador != 0: #verifica se não está no primeiro laço
                ...
                for i in range(len(self.__obstaculos)):
                    ...
                    obstaculos_rect[i].x = self.__obstaculos[i].x
                    obstaculos_rect[i].y = self.__obstaculos[i].y
                
                    #movimentacao do obstaculo, somente para baixo
                    self.__obstaculos[i].mover_baixo()

                    #desenha o rect do obstaculo [i]
                    pygame.draw.rect(screen, (0, 0, 0), obstaculos_rect[i], 4)

                    #cria a imagem do obstaculo [i]
                    screen.blit(self.__obstaculos[i].imagem, (self.__obstaculos[i].x, self.__obstaculos[i].y))
            ...
            self.__jogador.imagem = pygame.image.load(self.__jogador.sprites[contPrite-1]).convert_alpha()
            self.__jogador.imagem = pygame.transform.scale(self.__jogador.imagem, (80, 40))
            self.__jogador.imagem = pygame.transform.rotate(self.__jogador.imagem, 90)
